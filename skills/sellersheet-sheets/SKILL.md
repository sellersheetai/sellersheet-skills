...
**Build (new tab / new workbook).** The workflow below + Final review gate. Brand defaults apply in full. **Route by size first:** a net-new workbook or heavy multi-tab build (more than ~15 write/format calls) with a local shell available → build the `.xlsx` locally and import it in ONE `start_drive_upload(convertTo=…)` call — see `reference/local-build-import.md`. Single-tab / light builds, or no local filesystem → the MCP workflow below. **Availability check:** the LIVE `start_drive_upload` tool docstring is the only authority on whether `convertTo` exists on the connected server — load it (ToolSearch) and look for the `convertTo` arg. Never rule this route out from memory, recall, or prior-session notes; a wrong "unavailable" call costs ~50 sequential write/format round-trips (a real 2026-07-17 build took ~25 min the slow way vs ~4 min via import).
