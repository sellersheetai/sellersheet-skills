...
| **`#ERROR!`** | Formula syntax broken, OR `SQL()` reading a range that touches `NOW()` / `RAND()` / `RANDARRAY()` / `RANDBETWEEN()`, OR **literal text starting with `=` written without the apostrophe escape** (e.g. a Notes cell `= ordered − refunded`). Read `effective_value.error.message` for the specific reason. | **Real bug — must fix.** | NOW-collision → per-column `ARRAYFORMULA`. Unescaped literal → rewrite as `'= ordered − refunded` (see `mcp-gotchas.md`). |
...
| **`#NAME?`** on a non-`SQL()` / non-`IMAGE()` formula | Typo in function name or referenced range doesn't exist — OR **prose accidentally written as a formula**: a text cell starting with `=` (e.g. `= date`) parsed under USER_ENTERED. | **Real bug — must fix.** | Check the formula text; verify named ranges. If the cell was meant as text, rewrite with a leading apostrophe (`'= date`). |
