---
name: image-gen
description_zh: "亚马逊产品图生成、listing 图片优化、主图/副图/套图、A+ 页面、学习竞品风格、改色变体图、图片合规检查。"
description_en: "Use when creating or optimizing Amazon product listing images or A+ Content for a store SKU or a whole…"
...
1. **Plan first, gate before spend.** Default to GATED phases: get operator sign-off on (a) the
   ...
   Image generation costs real money (Credits, billed per action) — never fan out 20 images on an
